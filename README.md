...
（このページはシンクライアント端末、仮想デスクトップではなくFAT PCで直接御覧ください）
...
> [!WARNING]
>
> 以下のライブデモは **皆様のパソコンのCPU, GPUをかなり暖かくします** ので、ご注意ください。
...
画面左上の　`(10, 20)`　`(20, 30)`　`(30, 30)`　をクリックしてネットワーク構成の規模を切り替えてみてください。
...
GPUを意識することなく、JavaScriptだけで3Dプログラミングを実現できています。

ですが、GLSLというシェーダー言語を組み合わせると、意図的にGPUを働かせることもできます。
...
JavaScriptはシングルスレッドで動きますので、パーティクルの位置を順番に計算していく都合、数に比例して時間がかかってしまいます。
一方、GPUを使う場合は全ての点の位置を同時に計算できますので、計算時間を飛躍的に短縮できます。
...
約5万個の点の位置をフレームごとにGPUで計算しています（ワイヤーフレームにすることで線を表示しているだけで、実際には線すら書いてません）。
