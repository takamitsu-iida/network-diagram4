...
このような接続構成を**5ステージCLOSネットワーク**と呼びます。

テキストで表現した図としてはものすごく分かりやすいと思うのですが、
パッと見ただけだと **何がどう繋がっているのか分かりづらい** です。
...
可視化している元データは（離散数学でいうところの）グラフですので、最短経路の可視化や、障害発生時の影響範囲の可視化なんかもできそうですし、
...
Three.jsを使うとJavaScriptだけで3Dプログラミングができて大変ありがたいです。
実際、前述のライブデモはGPUの存在を意識することなく書いてますが、
GLSLというシェーダー言語を組み合わせると、意図的にGPUを働かせることもできます。
...
約5万個のパーティクル（点）を風のベクトルに沿って動かしています。
...
自分でも、風予報の可視化ができないかな、と思って作りました。

Windyのように美しく表現するには美的センスが必要なので私にはできませんでしたが、
GPUを使って計算するそのやり方は、いろんなところに応用できそうです。
...
基本的なやり方は前述の風の可視化と同じで、約5万個の点の位置をフレームごとにGPUで計算して動かしています（点と点を繋いで線にしています）。

IPアドレスと地図上の位置を紐づけるGeoIPと組み合わせたらおもしろい可視化表現ができるかも、と思って作りましたが、思ったほど美しくできなかったので放置しています。
